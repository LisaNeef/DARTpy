...
def get_experiment_date_ranges(exp_name):
	...
	if exp_name == 'NODA':
		DR = daterange(date_start=datetime.datetime(2009,1,1,0,0,0), periods=31, DT='1D')
	if exp_name == 'ERPALL':
		DR = daterange(date_start=datetime.datetime(2009,1,1,0,0,0), periods=31, DT='1D')
	if exp_name == 'RST':
		DR = daterange(date_start=datetime.datetime(2009,1,1,0,0,0), periods=17, DT='1D')
	if exp_name == 'ERPRST':
		DR = daterange(date_start=datetime.datetime(2009,1,1,0,0,0), periods=17, DT='1D')

	# DART-WACCM runs performed at GEOMAR  
	if exp_name == 'PMO32':
		DR = daterange(date_start=datetime.datetime(2009,10,1,6,0,0), periods=31, DT='6H')
	if exp_name == 'W0910_NODA':
		DR = daterange(date_start=datetime.datetime(2009,10,1,12,0,0), periods=596, DT='6H')
	...
	if exp_name == 'NCAR_PMO_CONTROL':
		DR = daterange(date_start=datetime.datetime(2008,11,6,6,0,0), periods=72, DT='6H')
	...
	if exp_name == 'NCAR_FULL':
		DR = daterange(date_start=datetime.datetime(2009,1,1,6,0,0), periods=204, DT='6H')
